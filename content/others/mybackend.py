...
class LazyFilesystemBackend(BackendProtocol):
    ...
    def __init__(self):
        ...
        self.root_dir = os.path.join(cfg.ROOT_DIR, cfg.WORK_SPACE, self.thread_id)  # 拼接工作目录路径
        self.backend = None
...
'''
它在 Agent 里的核心作用：
给 Agent 提供一个「统一的文件 / 记忆存储后端」，让 Agent 读写文件、保存记忆时，完全不用关心底层存在哪里、怎么存。
它的实现方式：
用工厂模式，接收 Agent 运行时信息（runtime），返回一个遵循统一存储协议的后端实例（文件系统后端）。
'''
