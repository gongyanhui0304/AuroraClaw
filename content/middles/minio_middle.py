...
from utils.doc_utils import zip_utils
...
class MinioMiddle(AgentMiddleware):
    ...
    async def aafter_agent(self, state, runtime):
        ...
        if os.path.exists(session_path) and ou.get_max_update_time(session_path, [cfg.USER_UPLOAD_DIR]) > state['work_start_time']:  # 如果这个目录存在
            # 对比工作目录的更新时间于状态中的工作开始时间即可。
            # 1. 把线程目录打包成zip
            local_zip_path = await asyncio.to_thread(zip_utils.zip_folder, session_path)
            # 2. 上传minio
