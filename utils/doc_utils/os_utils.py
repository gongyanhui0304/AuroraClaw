    ...
    for file_name in os.listdir(dir_path):
        ...
        if os.path.isdir(file_path):
            if not (ignore_dir_names and file_name in ignore_dir_names):
                get_files_update_time(file_path, all_update_times=all_update_times)
        else:
            all_update_times.append(os.path.getmtime(file_path))
    return all_update_times


def get_max_update_time(file_path, ignore_dir_names=None):
    all_update_times = get_files_update_time(file_path, ignore_dir_names=ignore_dir_names)
    return max(all_update_times) if all_update_times else 0


if __name__ == "__main__":
    # tree_output = list_directory("../")
    # print(tree_output)
    print(get_files_update_time("./"))
